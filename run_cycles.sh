...
wait_for_cycle_completion() {
    ...
    echo "  ⏳ Waiting for cycle to complete (max ${MAX_WAIT_HOURS}h)..."
    ...
    while true; do
        ...
        if [ $ELAPSED -gt $MAX_WAIT_SECONDS ]; then
            echo "  ⚠️  Cycle exceeded maximum wait time (${MAX_WAIT_HOURS}h), assuming stuck. Moving on..."
            break
        fi
        ...
        if [ ! -z "$LAST_LOG" ]; then
            ...
            if [ ! -z "$CYCLE_END_TIME" ]; then
                ...
                if [ ! -z "$LOG_TIMESTAMP" ] && [ "$LOG_TIMESTAMP" -gt "$CYCLE_START_TS" ]; then
                    echo "  ✅ Cycle completed at $CYCLE_END_TIME"
                    break
                fi
            fi
        fi
        ...
        if [ $((ITERATION % 20)) -eq 0 ]; then
            echo "  ⏳ Still waiting... (${ELAPSED}s elapsed, ~$((ELAPSED / 60)) minutes)"
        fi
    done
}
...
while true; do
    ...
    if [ "$STATUS" = "success" ]; then
        echo "  ✅ Cycle triggered successfully"
        ...
        echo "  ⏸️  Waiting $INTERVAL_MINUTES minutes before next cycle..."
        sleep $((INTERVAL_MINUTES * 60))
    else
        echo "  ❌ Failed to trigger cycle: $RESPONSE"
        echo "  ⏸️  Waiting 5 minutes before retry..."
        sleep 300
    fi
done
